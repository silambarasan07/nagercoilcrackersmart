...
async function submitWhatsAppOrder() {
  ...
  const fieldsMap = {
    ...
    email: { el: emailEl, name: 'custEmail' },
  ...
  } else if (!/^[6-9][0-9]{9}$/.test(mobile.replace(/\D/g, ''))) {
  ...
  } else if (!/^[6-9][0-9]{9}$/.test(whatsapp.replace(/\D/g, ''))) {
  ...
  try {
    ...
    localStorage.setItem('lastWhatsAppUrl', waUrl);
  } catch (err) {
    console.error("Failed to save order invoice:", err);
  }

  // Auto launch WhatsApp with pre-filled order message (synchronous to avoid popup blocking)
  try {
    window.open(waUrl, '_blank');
  } catch (err) {
    console.warn("Auto-open WhatsApp blocked:", err);
  }

  // Send HTML Invoice Email via PHP asynchronously (non-blocking with keepalive)
  try {
  ...
  // Direct window navigation to Invoice page (works reliably on Desktop, Tablet & Mobile)
